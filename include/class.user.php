...
class USER {
    ...
    public function Subject($amount, $type_of_account, $purpose, $interval, $comment, $email) {
        try {                                                                               
            $stmt = $this->conn->prepare("INSERT INTO sub(amount,type_of_account,purpose,`interval`,comment, email)
                VALUES (:amount, :type_of_account, :purpose, :interval, :comment, :email)");
            $stmt->bindparam(":amount", $amount);
